(function($, document, window){
	...
	$(document).ready(function(){
		...
		$("[data-background]").each(function(){
			...
			if( retina ){
				// Check if the image path contains dummy images and handle retina versions properly
				var retinabg = bg.replace(".jpg","@2x.jpg");
				// Test if retina image exists before using it
				var img = new Image();
				img.onload = function() {
					$(this).css("background-image","url("+retinabg+")");
				}.bind(this);
				img.onerror = function() {
					// Fallback to regular image if @2x version doesn't exist or fails to load
